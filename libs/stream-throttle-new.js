...
function process(self, chunk, pos, done) {
  ...
  console.log("Processing chunk size",chunk.length,"slice",slice.length);
    self.bucket.removeTokens(slice.length, function(err) {
        if (err) {
            done(err);
            return;
        }
      console.log("Pushed slice of size",slice.length);
        self.push(slice);
...
function ThrottleGroup(opts) {
    ...
    let this2 = this;
    this.sendBurst = function(bytes){
      this2.bucket.content += bytes;
    }
}
